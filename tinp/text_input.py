...
class TextInput(Widget):
    ...
    def _render_text_with_cursor(self) -> List[Union[str, Text, Tuple[str, Style]]]:
        ...
        # trim the string to fit within the height of the widget



        # convert the cursor to be relative to this view
        cursor_relative_position = self._cursor_position
        return [
    ...
    def _cursor_left(self):
        ...
        if self.value.split("\n")[-1].count("> ") == 1 and self.value.split("\n")[-1].endswith("> "):
                return
    ...
    def _key_backspace(self):
        ...
        if self._cursor_position > 0:
            # if there is only one "> " left, don't delete it (hint: self.value.split("\n")[-1])
            # count the number of "> " in the value
            if self.value.split("\n")[-1].count("> ") == 1 and self.value.split("\n")[-1].endswith("> "):
                return
            self.value = (
